...
class WikipediaCrawler:
    ...
    async def get_country_urls(self):
    ...
    async def parse_country_page(self, url):
        async with aiohttp.ClientSession() as session:
...
async def main():
    ...
    country_urls = await crawler.get_country_urls()

    tasks = [crawler.parse_country_page(url) for url in country_urls[1:]]
